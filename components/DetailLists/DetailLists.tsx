...
import { Ability, Move, PokemonType, Stat } from './types';
  ...
  return (
    <Box>
      {stats.map((stat: Stat, index: number) => (
        <Text key={`stat-${index}`}>
          <strong>{getStatName(stat)}:</strong> {stat.base_stat}
        </Text>
      ))}
    </Box>
  );
